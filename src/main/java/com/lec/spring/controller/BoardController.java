...
public class BoardController {
    ...
    public String writeOk(
        ...
        //validation 에러 가 있었다면 redirect 한다!
        if(result.hasErrors()) {
            showErrors(result);
            ...
            for(FieldError err : result.getFieldErrors()){
                redirectAttributes.addFlashAttribute("error_" + err.getField(), err.getCode());
                                                                // error_user, error_subject...
            }

            return "redirect:/board/write"; // redirect 는 GET 방식의 request 임
        }
...
//        model.addAttribute("list", boardService.list());
    ...
    public String updateOk(
            @RequestParam Map<String, MultipartFile> files, //새로 추가된 첨부파일들
            Long[] delfile, //삭제할 파일 목록
            @Valid Post post
